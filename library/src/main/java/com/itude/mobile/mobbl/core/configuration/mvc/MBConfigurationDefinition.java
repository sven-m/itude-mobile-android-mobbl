...
public class MBConfigurationDefinition extends MBDefinition implements MBIncludableDefinition {
    ...
    public void addDomain(MBDomainDefinition domain) {
        if (_domainTypes.containsKey(domain.getName())) {
            MBLog.w(MBConstants.APPLICATION_NAME, "Domain definition overridden: multiple definitions for domain with name " + domain.getName());
        }
    ...
    public void addAction(MBActionDefinition action) {
        if (_actionTypes.containsKey(action.getName())) {
            MBLog.w(MBConstants.APPLICATION_NAME, "Action definition overridden: multiple definitions for action with name " + action.getName());
        }
    ...
    public void addPage(MBPageDefinition page) {
        if (_pageTypes.containsKey(page.getName())) {
            MBLog.w(MBConstants.APPLICATION_NAME, "Page definition overridden: multiple definitions for page with name " + page.getName());
        }
    ...
    public void addDialog(MBDialogDefinition dialog) {
        if (_dialogs.containsKey(dialog.getName())) {
            MBLog.w(MBConstants.APPLICATION_NAME, "Dialog definition overridden: multiple definitions for dialog with name " + dialog.getName());
        }
    ...
    public void addAlert(MBAlertDefinition alert) {
        if (_alerts.containsKey(alert.getName())) {
            MBLog.w(MBConstants.APPLICATION_NAME, "Alert definition overridden: multiple definitions for alert with name " + alert.getName());
        }
