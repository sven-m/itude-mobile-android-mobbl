    ...
    public boolean processElement(String elementName, Map<String, String> attributeDict) {
        ...
        if (elementName.equals("EndPoints")) {
            ...
            for (Map.Entry<String, String> attribute : attributeDict.entrySet()) {
                if ("documentIn".equals(attribute.getKey()))
                    endpointDef.setDocumentIn(attribute.getValue());
                else if ("documentOut".equals(attribute.getKey()))
                    endpointDef.setDocumentOut(attribute.getValue());
                else if ("endPoint".equals(attribute.getKey()))
                    endpointDef.setEndPointUri(attribute.getValue());
                else if ("cacheable".equals(attribute.getKey()))
                    endpointDef.setCacheable(Boolean.parseBoolean(attribute.getValue()));
                else if ("ttl".equals(attribute.getKey()))
                    endpointDef.setTtl(Integer.parseInt(attribute.getValue()));
                else if ("timeout".equals(attribute.getKey()))
                    endpointDef.setTimeout(Integer.parseInt(attribute.getValue()));
