...
public abstract class MBOutcomeTask<Result> implements Runnable {
    ...
    protected MBIndicator showIndicator() {
        if (getOutcome().getNoBackgroundProcessing())
            return MBIndicator.show(Type.none, getOutcome());
        if ("exception".equals(getOutcome().getOutcomeName()))
            return MBIndicator.show(Type.none, getOutcome());
        if (getThreading() != Threading.BACKGROUND)
            return MBIndicator.show(Type.none, getOutcome());
    ...
    public final void run() {
        ...
        try {
            MBLog.d(this.getClass().getSimpleName(), "Running outcome task for " + getOutcome());
            execute();
